package com.example.android.susmap;
...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String APP_FOLDER_NAME = "SusMap";
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        switch(item.getItemId()){
            case R.id.id_mylocation:
                ...
                break;
            case R.id.id_moninav:
                if (mDestLocationData == null) {
                    Toast.makeText(this, "请长按地图设置目标地点", Toast.LENGTH_SHORT).show();
                }else {
                    routeplanToNavi(false);
                }
                break;
            case R.id.id_startnav:
                if (mDestLocationData == null) {
                    Toast.makeText(this, "请长按地图设置目标地点", Toast.LENGTH_SHORT).show();
                }
                else { routeplanToNavi(true);}
                break;
    ...
    private void initSetting() {
       ...
       Bundle bundle = new Bundle();
        // 必须设置APPID，否则会静音
        bundle.putString(BNCommonSettingParam.TTS_APP_ID, "10349243");
